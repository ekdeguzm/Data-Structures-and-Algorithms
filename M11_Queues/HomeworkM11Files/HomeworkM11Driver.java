...
public class HomeworkM11Driver {
	...
	public static void main(String[] args) {
		...
		testClientSplice(new Integer[] { 7 }, new Integer[] { 4, 9, 1, 3 },
				"splice non-empty queue to singleton queue");
		testClientSplice(new Integer[] { 1, 3, 2, 4 }, new Integer[] { 6, 9, 3 },
				"splice non-empty queue to non-empty queue");
		...
		testArraySplice(new Integer[] { 1, 3, 2, 4 }, new Integer[] { 6, 9, 3 },
				"splice non-empty queue to non-empty queue");
		testArraySplice(new String[] { "a", "b", "c", "d" }, new String[] { "d", "e", "d" }, "splice Strings");

		// test the wrap-around nature of ArrayQueue
		// for these tests, extra elements are enqueued and dequeued to force the
		// contents to wrap around the array
		...
		testLinkedSplice(new Integer[] { 7 }, new Integer[] { 4, 9, 1, 3 },
				"splice non-empty queue to singleton queue");
		...
		testLinkedSplice(new Integer[] { 1, 3, 2, 4 }, new Integer[] { 6, 9, 3 },
				"splice non-empty queue to non-empty queue");
		testLinkedSplice(new String[] { "a", "b", "c", "d" }, new String[] { "d", "e", "d" }, "splice Strings");
		testLinkedSplice(new Integer[] { 4, 2, 3 }, new Integer[] { 1 }, "splice singleton queue to non-empty queue");

		// test to make sure that the chains aren't actually "hooked together" but that
		// the actual elements are spliced to q1
	...
	private static enum SpliceType {
		CLIENT, ARRAY, LINKED
	};
	...
	public static void testArraySpliceWrap(Integer[] contents1, Integer[] contents2, String testDescription) {
		...
		q1.dequeue();
		q1.dequeue();
		q1.dequeue(); // forcing the contents to wrap around the array
		q1.enqueue(contents1[1]);
		q1.enqueue(contents1[2]);
		q1.enqueue(contents1[3]);
		...
		q2.dequeue();
		q2.dequeue();
		q2.dequeue(); // forcing the contents to wrap around the array
		q2.enqueue(contents2[1]);
		q2.enqueue(contents2[2]);
		q2.enqueue(contents2[3]);
	...
	public static void testLinkedSpliceConnected() {
		...
		System.out.println(
				"Next: q2.enqueue(5) and by q1.enqueue(6). Q1 should hold [1, 2, 3, 4, 6] and Q2 should hold [3, 4, 5]");
		...
		if (q2.getFront() != 5) {
			...
			System.out.println(
					"*****TEST FAILED: Splice should add elements to q1. It should not link the chains together.");
			System.out
					.println("   As written, changes to q1 can affect q2 after the splice is complete and vice versa.");
		} else {
	...
	public static <T> void testSplice(T[] contents1, QueueInterface<T> q1, T[] contents2, QueueInterface<T> q2,
			boolean build, String testDescription, SpliceType type) {
		...
		if (type != SpliceType.LINKED) {
			...
			T expectedLastQ1 = expectedResultContents.length == 0 ? null
					: expectedResultContents[expectedResultContents.length - 1];
			...
			if (!q1FirstMatches || !q1LastMatches || !q2FirstMatches || !q2LastMatches) {
				allTestsPassed = false;
				System.out.println(
						"*****TEST FAILED. firstNode or lastNode doesn't match for at least one queue after the splice for test: "
								+ testDescription);
			}
		}
		...
		if (actualSizeQ1 != expectedSizeQ1 || actualSizeQ2 != expectedSizeQ2) {
			allTestsPassed = false;
			System.out.println("*****TEST FAILED. Actual size of Q1 or Q2 after the splice is incorrect for test: "
					+ testDescription);
			System.out.println("   Expected size of Q1 after slice=" + expectedSizeQ1
					+ " and expected size of Q2 after slice=" + expectedSizeQ2);
		}
	...
	private static <T> void testGetSecond(QueueInterface<T> queue, QueueInterface<T> printQueue, T expectedResult,
		...
		try {
			...
			// Normally this is bad practice to catch a general exception object. Don't do
			// this!!
			// I had to do this here to make this general since you can choose different
			// kinds of exceptions for this question.
		...
		if (exceptionThrown) {
			System.out.println(
					"  Actual getSecond behavior: " + "exception thrown of type " + exception.getClass().getName());
		} else {
		...
		if (expectedResult == null) {
			if (!(actualResult == null || exceptionThrown)) {
				...
				System.out
						.println("     Expected getSecond contents are either null or an exception should be thrown.");
			}
		} else {
	...
	public static void testArrayGetSecondSpecialCases() {
		...
		queue.dequeue();
		queue.dequeue();
		queue.dequeue(); // forcing the contents to wrap around the array
		queue.enqueue(2);
		queue.enqueue(3);
		queue.enqueue(4);
