...
public class SearchSortHomeworkDriver {
	...
	public static double sortedness(Comparable[] array) {
		...
		return (double) sortedPairs / totalPairs;
	}
	...
	public static double sortedness(Node<Comparable> node) {
		...
		return sortednessHelper(node, 0, 0);
	}

	private static double sortednessHelper(Node<Comparable> currentNode, int sortedPairs, int totalPairs) {

		if (currentNode.getNextNode() == null) {
			return (totalPairs == 0) ? 1.0 : (double) sortedPairs / totalPairs;
		}

		if (currentNode.getData().compareTo(currentNode.getNextNode().getData()) <= 0) {
			sortedPairs++;
		}

		return sortednessHelper(currentNode.getNextNode(), sortedPairs, totalPairs + 1);
	}
	...
	public static void main(String[] args) {
		...
		// parameter 2: the expected sortedness, as a double (e.g., 1 means 100% sorted;
		// 0.5 means 50% sorted)
		...
		testArraySortedness(new String[] { "cat", "ant", "dog", "egg" }, 0.66667,
				"partially sorted even length of Strings");
		...
		testLinkedNodeSortedness(new String[] { "cat", "ant", "dog", "egg" }, 0.66667,
				"partially sorted even length of Strings");
	...
	public static <T extends Comparable<? super T>> void testArraySortedness(T[] array, double expectedSortedness,
	...
	public static <T extends Comparable<? super T>> void testLinkedNodeSortedness(T[] array, double expectedSortedness,
			String testDescription) {
		NumberFormat formatter = NumberFormat.getPercentInstance();
