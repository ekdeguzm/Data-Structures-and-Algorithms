...
public class HomeworkM1Driver {
	...
	public static void main(String[] args) {
		...
		DigitalReceipt digitalReceipt1 = new DigitalReceipt(bookStore, 50, 525.25, "bxxbxx3x",
				"sir.reads.a.lot@books.com");
		DigitalReceipt digitalReceipt2 = new DigitalReceipt(bookStore, 1, 4.87, "uuu3fqqq", "mark.darcy@gmail.abc");
		DigitalReceipt digitalReceipt3 = new DigitalReceipt(electronicStore, 2, 139.21, "xxx2m2xx",
				"dr.frankenstein@monster.com");
		DigitalReceipt digitalReceipt4 = new DigitalReceipt(groceryStore1, 18, 78.41, "ybbby99y", "t@iced.com");
		Receipt[] receipts = { receipt1, receipt2, receipt3, receipt4, digitalReceipt1, digitalReceipt2,
				digitalReceipt3, digitalReceipt4 };
		...
		// the testEquals method compares one of the receipts to the testReceipt to
		...
		testReceipt = new Receipt(receipt2.getStore(), receipt2.getNumberOfItems(), receipt2.getTotal(),
				receipt2.getReceiptID());
		testEquals(receipt2, testReceipt, true, "logically equivalent receipt");

		Store testStore = new Store(receipt3.getStore().getName().toUpperCase(),
				receipt3.getStore().getLocation().toLowerCase());
		...
		testReceipt = new Receipt(receipt4.getStore(), receipt4.getNumberOfItems(), receipt4.getTotal(),
				receipt4.getReceiptID().toUpperCase());
		...
		testReceipt = new Receipt(receipt2.getStore(), receipt2.getNumberOfItems() + 1, receipt2.getTotal(),
				receipt2.getReceiptID());
		testEquals(receipt2, testReceipt, false, "receipt with a different number of items");

		testReceipt = new Receipt(receipt3.getStore(), receipt3.getNumberOfItems(), receipt3.getTotal() * 2,
				receipt3.getReceiptID());
		testEquals(receipt3, testReceipt, false, "receipt with a different total spent");

		testReceipt = new Receipt(receipt4.getStore(), receipt4.getNumberOfItems(), receipt4.getTotal(),
				receipt4.getReceiptID().substring(1));
		...
		// the method then calls ArrayList.contains to test if the receipt is equal to
		// one on the list
		...
		// parameters 2-4: the ones passed into the idMeetsCriteria method (primaryChar,
		// followingChar, and countOfFollowingAfterPrimary)
		// parameter 5: the expected result (true if the receipt meets the criteria,
		// false otherwise)
		...
		testReceiptCriteria("VoEVooVV", 'E', 'V', 3, true,
				"exactly 3 V's (followingChar) after E (primaryChar); note: also 1 extra V before E that is irrelevant");
		testReceiptCriteria("00g0p0gp", 'g', 'p', 2, true,
				"exactly 2 p's (followingChar) after the first g (primaryChar); note: also 1 extra g that is irrelevant");
		testReceiptCriteria("y1q111q1", 'q', 'y', 0, true,
				"exactly 0 y's (followingChar) after the first q (primaryChar); note: also 1 y before the q that is irrelevant");
		testReceiptCriteria("a11a1111", 'a', 'z', 0, true,
				"exactly 0 z's (followingChar) after the first a (primaryChar)");

		testReceiptCriteria("ooFoDDoo", 'F', 'D', 3, false,
				"only 2 D's (followingChar) after the F (primaryChar) instead of 3");
		testReceiptCriteria("AooZoAoo", 'Z', 'A', 2, false,
				"only 1 A (followingChar) after the Z (primaryChar); note: there is a second A but it comes before the Z so it is irrelevant");
		testReceiptCriteria("1111pqqq", 'p', 'q', 2, false,
				"3 q's (followingChar) after the p (primaryChar) instead of 2");
		testReceiptCriteria("1111m111", 'c', 'm', 1, false, "no c (primaryChar)");
		testReceiptCriteria("obbboooo", 'b', 'b', 3, false,
				"the first b is the primaryChar; there are then only 2 *other b's* (that are in the count of followingChar) after that instead of 3");
		...
		// parameter 2: the expected result (true if the email is valid, false
		// otherwise)
		// parameter 3: a description of the test
		/*
		 * String validEmailDescription =
		 * "at least one letter somewhere before the @ and exactly three characters after the last period"
		 * ; testEmailValidator("sir.reads.a.lot@books.com", true,
		 * validEmailDescription); testEmailValidator("mark.darcy@gmail.abc", true,
		 * validEmailDescription); testEmailValidator("dr.frankenstein@monster.com",
		 * true, validEmailDescription); testEmailValidator("t@iced.gov", true,
		 * validEmailDescription); testEmailValidator("ina@hurry.edu", true,
		 * validEmailDescription); testEmailValidator("morris?@better.org", true,
		 * validEmailDescription); testEmailValidator("!hello@acb.giv", true,
		 * validEmailDescription);
		 * 
		 * testEmailValidator("@abc.xyz", false, "no characters before @");
		 * testEmailValidator("abc.com", false, "no @"); testEmailValidator("j@com",
		 * false, "no period"); testEmailValidator("jessica@com", false, "no period");
	 ...
	 * ignore them. :)
	 */
	public static void testEquals(Receipt receipt, Receipt testReceipt, boolean expectedResult,
	...
	public static void testEqualsMethod(Receipt[] receipts) {
		...
		for (Receipt receipt : receipts) {
			...
			if (receipt instanceof DigitalReceipt) {
				copyReceipt = new DigitalReceipt(receipt.getStore(), receipt.getNumberOfItems(), receipt.getTotal(),
						new String(receipt.getReceiptID()), ((DigitalReceipt) receipt).getEmail());
			} else {
				copyReceipt = new Receipt(receipt.getStore(), receipt.getNumberOfItems(), receipt.getTotal(),
						new String(receipt.getReceiptID()));
			}
		...
		for (Receipt receipt : receipts) {
			...
			if (contains != true) {
				allTestsPassed = false;
				System.out.println(
						"**********TEST FAILED: equals method failed when testing equality of receipt: " + receipt);
	...
	public static void testReceiptCriteria(String receiptID, char firstChar, char secondChar, int timesAfter,
		...
		System.out.println("\nID=" + receiptID + "  firstTarget=" + firstChar + "  secondTarget=" + secondChar
