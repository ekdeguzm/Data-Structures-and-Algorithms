...
public class HomeworkM14Driver {
	...
	public static void main(String[] args) {
		...
		equivalentStudent = new Student(Integer.valueOf(123), new String("Sam"), new String("Studier"),
				Boolean.valueOf(true));
		testEqualsHashCode(testStudent, equivalentStudent, true, true, "equivalent students");

		Student differentStudent = new Student(123, "Sam", "Studier", false);
		testEqualsHashCode(testStudent, differentStudent, false, false,
				"non-equivalent students: different tuition status");
		...
		testEqualsHashCode(testStudent, differentStudent, false, false,
				"non-equivalent students: different first name");
		...
		testEqualsHashCode(testStudent, differentStudent, false, false,
				"non-equivalent students: same id, different names");
		...
		for (int tableSize : tableSizes) {
			...
			Voter[] theVoters = { new Voter(912, "Delta Demm"), new Voter(670, "Riley Repp"),
					new Voter(381, "Ivan Indie"), new Voter(497, "Greta Green"), new Voter(301, "Liz Libert"),
					new Voter(541, "Ursula Undec") };
			...
			for (Voter voter : theVoters) {
				...
				if (retrievedVoter == null || retrievedVoter.getId() != (voter.getId())
						|| !retrievedVoter.getName().equals((voter.getName()))) {
					allTestsPassed = false;
			...
			if (retrievedVoter != null) {
				allTestsPassed = false;
				System.out.println(
						"*****TEST FAILED. null should be returned when attempting to retreive a voter not in the table.");
			}
			...
			Voter[] duplicateVoters = { new Voter(912, "Delta Demm"),
					new Voter(Integer.valueOf(912), new String("Delta Demm")), new Voter(541, "Ursula Undec"),
					new Voter(Integer.valueOf(541), new String("Ursula Undec")) };
	 ...
	 * 
	 */
	public static void testEqualsHashCode(Student studentA, Student studentB, boolean expectedEqualsResult,
			boolean expectedHashResult, String testDescription) {
		...
		if (expectedEqualsResult == true) {
			...
			if (expectedHashResult != actualHashResult) {
				System.out.println(
						"***Note: these non-equivalent objects have the same hash code. This doesn't violate the contract, but you might review your hashCode method to see if it can be improved!");
			}
			if (actualEqualsResult == true && actualHashResult == false) {
				allTestsPassed = false;
				System.out.println(
						"*****TEST FAILED: hash code test. Your equals method says these students are equal (which is incorrect).");
				System.out.println(
						"     Since your equals method returned true, your hash code method should have also returned true.");
			}
		}
